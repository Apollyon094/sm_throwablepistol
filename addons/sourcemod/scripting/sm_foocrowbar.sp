...
public OnPluginStart()
{
	...
	g_hCvarVersion = CreateConVar("sm_foocrowbar_version", VERSION, "[foo] bar's crowbar version",  FCVAR_NOTIFY|FCVAR_DONTRECORD);
	SetConVarString(g_hCvarVersion, VERSION);
	
	g_hDebug = CreateConVar("sm_foocrowbar_debug", "0", "Enable console debugging");
	HookConVarChange(g_hDebug, OnConVarChanged);

	g_hCrowbarExplodeMagnitude =  CreateConVar("sm_foocrowbar_explodedamage", "80", "Thrown Crowbar Explosion Magnitude.  Set to 0 to disable");
	HookConVarChange(g_hCrowbarExplodeMagnitude, OnConVarChanged);

	g_hCrowbarExplodeRadius = CreateConVar("sm_foocrowbar_exploderadius", "0", "Thrown Crowbar Explosion Radius Override.  Leave as 0 to scale to sm_foocrowbar_explodedamage");
	HookConVarChange(g_hCrowbarExplodeRadius, OnConVarChanged);

	g_hCrowbarThrownDamage = CreateConVar("sm_foocrowbar_throwndamage", "80", "Damage a Thrown Crowbar should inflict");
	HookConVarChange(g_hCrowbarThrownDamage, OnConVarChanged);
	
	g_hCrowbarMaxSpawn = CreateConVar("sm_foocrowbar_maxperspawn", 	"5", "Maximum throwing crowbars per spawn.  0 = unlimited. -1 = none");
	HookConVarChange(g_hCrowbarMaxSpawn, OnConVarChanged);

	g_hCrowbarMaxLife = CreateConVar("sm_foocrowbar_maxlife", "30", "Maximum seconds thrown crowbars (that don't explode) should stick around before dying");
	HookConVarChange(g_hCrowbarMaxLife, OnConVarChanged);

	g_hCrowbarMinTime = CreateConVar("sm_foocrowbar_mintime", "1", "Minimum seconds required between thrown crowbars");
	HookConVarChange(g_hCrowbarMinTime, OnConVarChanged);

	g_hCrowbarDonatorLevel = CreateConVar("sm_foocrowbar_donator_level", "0",  "Minimum donator level required to use throwing crowbars/stunsticks. 0 = don't use donator system");
	HookConVarChange(g_hCrowbarDonatorLevel, OnConVarChanged);
	
	g_hCrowbarDonatorMaxSpawn = CreateConVar("sm_foocrowbar_donator_maxperspawn", "0", "Maximum throwing crowbars per spawn for donators.  0 = unlimited");
	...
	PrintToServer("foo's crowbar %s loaded", VERSION);
}
...
public Action:Event_PlayerSpawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if(Client_HasWeapon(client, "weapon_crowbar")){
		has_crowbar = true;
	}

	if(Client_HasWeapon(client, "weapon_stunstick")){
		has_stunstick = true;
	}
	
	if(has_crowbar==true && has_stunstick==true ) {
		Format(weaponString, sizeof(weaponString), "crowbars and stunsticks");
	} else if(has_crowbar==true) {
		Format(weaponString, sizeof(weaponString), "crowbars");
	} else if(has_stunstick == true ) {
		Format(weaponString, sizeof(weaponString), "stunsticks");
	} else {
		Format(weaponString, sizeof(weaponString), "crowbars");	// this shouldn't happen.
	}
...
public Action:OnTakeDamage(victim, &attacker, &inflictor, &Float:damage, &damagetype, &weapon, Float:damageForce[3], Float:damagePosition[3], damagecustom)
{
	...
	if(StrEqual(attackerWeapon, "prop_physics")){
		if(gotTrie == false){
			PrintToServer("sm_foocrowbar: OnTakeDamage: BAD TRIE!");
			return Plugin_Continue;
		}
...
public Action:OnPlayerRunCmd(client, &iButtons, &Impulse, Float:fVelocity[3], Float:fAngles[3], &iWeapon)
{
	...
	if(iActiveWeapon != -1 && IsValidEntity(iActiveWeapon) && IsValidEdict(iActiveWeapon) ) {
		...
		if((StrEqual(weapon, "weapon_crowbar") || StrEqual(weapon, "weapon_stunstick")) && (iButtons & IN_ATTACK2 )) {
			...
			if(StrEqual(weapon, "weapon_crowbar")) {
				ThrowCrowbar(client, MELEE_CROWBAR);
			} else if(StrEqual(weapon, "weapon_stunstick")){
				ThrowCrowbar(client, MELEE_STUNSTICK);
			}
...
public explode(entity)
{
	...
	if(ent<0){
		PrintToServer("Failed to crate env_physexplosion!");
		return false;
	}
...
ThrowCrowbar(client, meleeType)
{
	...
	if(meleeType==MELEE_CROWBAR){
		SetEntityModel(ent, "models/weapons/w_crowbar.mdl");
	} else if(meleeType==MELEE_STUNSTICK) {
		SetEntityModel(ent, "models/weapons/w_stunbaton.mdl");
	} else {
		PrintToServer("sm_foocrowbar: Unknown melee weapon in ThrowCrowbar: %d", meleeType);
		return;
	}
